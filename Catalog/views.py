...
class AuthorCreateView(LoginRequiredMixin, generic.ListView):
    model = Author


class AuthorUpdateView(LoginRequiredMixin, generic.ListView):
    model = Author


class AuthorDeleteView(LoginRequiredMixin, generic.ListView):
    model = Author
...
BookInstance.objects.filter(status__exact='a').order_by('book__title')
